...
export function useChatConversation() {
  ...
  useEffect(() => {
    const handleProgress = (data: any) => {
      ...
      if (!isTechnicalNoise) {
        setMessages(prev => {
          // If last message is a thinking message, update it instead of adding new one
          if (prev.length > 0) {
            const last = prev[prev.length - 1];
            if (last.type === 'thinking') {
              // Accumulate meaningful progress updates
              const updatedContent = last.content.includes(progressText)
                ? last.content
                : `${last.content}\n${progressText}`;
              return [...prev.slice(0, -1), last.withContent(updatedContent)];
            }
          }
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    if (!isConnected) {
      ...
      try {
        ...
        setMessages(prev => [...prev, ChatMessage.createThinkingMessage('Starting PRD generation...')]);
      } catch (error) {
        setIsGenerating(false);
        setIsConnected(false);
  ...
  const answerClarification = useCallback(async (answerText: string) => {
    ...
    setMessages(prev => [...prev, ChatMessage.createThinkingMessage('Processing your answers...')]);
